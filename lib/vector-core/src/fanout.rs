...
mod tests {
    ...
    async fn fanout_strips_finalizers_for_marked_senders() {
        ...
        let (sender_auth, mut receiver_auth) = build_sender_pair(4).await;
        let (sender_non_auth, mut receiver_non_auth) = build_sender_pair(4).await;
    ...
    async fn fanout_strip_flag_preserved_across_replace() {
        ...
        let (new_sender, mut new_receiver) = build_sender_pair(4).await;
        control
